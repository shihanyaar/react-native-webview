...
export default class Injection extends Component<Props, State> {
  ...
  private myRef = React.createRef<WebView>();

  componentDidMount() {
    console.log('aaa', this.myRef.current.injectJavaScript);
    console.log('bbb', this.myRef.current.getCommands().setCookies);
    console.log('bbb', this.myRef.current.setCookies);
    setTimeout(() => {
      this.myRef.current.setCookies(`alert("two");`);
    }, 2000);
  }
  ...
  render() {
    return (
      <ScrollView>
        ...
        <Text>
          ⚠️ If either of the two iframes become coloured cyan, then for that
          given frame, JS injection succeeded after the content loaded, but
          didn't occur before the content loaded - please note that for iframes,
          this may not be a test failure, as it is not clear whether we would
          expect iframes to support an injection time of beforeContentLoaded
          anyway.
        </Text>
        <Text>
          ⚠️ If "Names of iframes that called beforeContentLoaded: " is [], then
          see above.
        </Text>
        <Text>
